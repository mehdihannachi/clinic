...
materialAdmin.controller('chatSingleHistoryController', function($location, $sce, $uibModal, $timeout, $scope, $http, $resource, $window, $state, $sessionStorage, $uibModal, $rootScope, growlService, Socket, usSpinnerService) {
    ...
    Socket.on('newMessage', function(data) {
        if ($location.path() == '/single-chat') {
            if (data.to == $sessionStorage.userProfile.user_id && data.from == $scope.discussion.correspondant.user_id) {
                ...
                if (data.from == $scope.discussion.correspondant.user_id) {
                    ...
                    data.fromUser = $scope.userProfile;
            ...
            else if (data.to == $sessionStorage.userProfile.user_id) {
               $scope.$emit('customEvent');
            }
        }
    });
    ...
    //init the controller
    $rootScope.initSingleChatHistory = function() {
        $scope.correspondant;
        $scope.userProfile = $sessionStorage.userProfile;
        $http.get('/chat/discussionHistory/' + $sessionStorage.discussion.id,{
            ...
            $http.post('/chat/seenOnMessages', {
                ...
                messagesTo: $sessionStorage.userProfile
            }).success(function(response) {
                //$scope.$emit('customEvent');
                $scope.discussion.messages.forEach(function(message) {
                    if (message.from == $scope.discussion.correspondant.user_id) {
                        ...
                        message.fromUser = $scope.userProfile;
    ...
    $scope.newChat = function(user_id) {
        $http.post('/chat/new/', {
            userProfile: $sessionStorage.userProfile,
    ...
    $scope.sendMessage = function() {
        if ($scope.messageText != '') {
            ...
            discussion.users.forEach(function(user) {
                if (user._id != $sessionStorage.userProfile.user_id)
            ...
            $http.post('/chat/sendMessage/', {
                ...
                if (response.message.from == $scope.discussion.correspondant.user_id) {
                    ...
                    response.message.fromUser = $scope.userProfile;
